...
{
    scmp_filter_ctx sec;
    sec = seccomp_init(SCMP_ACT_KILL);
    seccomp_rule_add(sec, SCMP_ACT_ALLOW, SCMP_SYS(read), 0);
    seccomp_rule_add(sec, SCMP_ACT_ALLOW, SCMP_SYS(write), 0);
    seccomp_rule_add(sec, SCMP_ACT_ALLOW, SCMP_SYS(exit_group), 0);
    seccomp_rule_add(sec, SCMP_ACT_ALLOW, SCMP_SYS(newfstatat), 0);
    seccomp_rule_add(sec, SCMP_ACT_ALLOW, SCMP_SYS(openat),0);
    seccomp_rule_add(sec, SCMP_ACT_ALLOW, SCMP_SYS(sendfile),0);
    seccomp_rule_add(sec, SCMP_ACT_ALLOW, SCMP_SYS(fstat),0);
    seccomp_rule_add(sec, SCMP_ACT_ALLOW, SCMP_SYS(close),0);
    seccomp_rule_add(sec, SCMP_ACT_ALLOW, SCMP_SYS(exit),0);
    seccomp_load(sec);
...
void sys(const uint8_t syscall_number)
{
    switch (syscall_number)
    {
        ...
        case 0x3c:
            if(r1==0)
            {
                std::cout<<"Correct Password\nThe flag is:"<<std::endl;
                std::ifstream flag_file ("/home/ctf/flag.txt");
                assert(flag_file.is_open());
                std::string flag;
                getline(flag_file,flag);
